...
export class Analytics {
  ...
  async track(namespace: string, event: object = {}, opts?: TrackOptions) {
    ...
    await getRedisClient().hincrby(key, JSON.stringify(event), 1)
    if (!opts?.persist) await getRedisClient().expire(key, this.retention)
  }
  ...
  async retrieve(namespace: string, date: string) {
    const res = await getRedisClient().hgetall<Record<string, string>>(
