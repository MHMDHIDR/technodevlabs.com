    ...
    <header className='fixed top-0 left-0 w-full z-[100] bg-white dark:bg-neutral-900 py-2 px-4 sm:px-10 shadow-sm shadow-purple-900'>
      <nav className='container mx-auto flex items-center justify-between'>
        ...
        <div className='relative'>
          ...
          />
          <label htmlFor='menu-toggle' className='block sm:hidden cursor-pointer z-50'>
            <svg
              className='w-6 h-6'
              viewBox='0 0 24 24'
              fill='none'
          ...
          <div
            className='absolute top-full right-0 mt-2 bg-white dark:bg-neutral-900 transition-all duration-300 ease-in-out opacity-0 -translate-y-2 invisible sm:relative sm:opacity-100 sm:translate-y-0 sm:visible sm:bg-transparent sm:mt-0 peer-checked:opacity-100 peer-checked:translate-y-0 peer-checked:visible w-48 sm:w-auto'
            id='menu'
          >
            <div className='flex flex-col sm:flex-row sm:items-center gap-4 sm:p-0 border shadow-md md:shadow-none md:border-none'>
...
function NavLink({ href, label }: { href: string; label: string }) {
  return (
    <Link
      href={href}
      className='font-medium text-gray-600 hover:text-gray-800 dark:text-neutral-400 dark:hover:text-neutral-500 focus:outline-purple-900 p-4 hover:bg-purple-900/10 transition-colors'
    >
      {label}
    </Link>
  )
